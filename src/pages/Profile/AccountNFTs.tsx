...
import { MyCreateNftListProp, useMyCreateNftAccountList, useNftAccountInfo } from 'hooks/useBackedNftCallback'
...
export default function AccountNFTs({ account }: { account: string }) {
  ...
  return (
    ...
    >
      ...
      {tabValue === 1 && <CreateNft6551 myCreateNftList={myCreateNftList} page={page_c} />}
    </ContainerWrapper>
  )
}
...
function CreateNft6551({
  ...
  myCreateNftList: MyCreateNftListProp[]
  page: {
  ...
  const [viewAll, setViewAll] = useState(false)
  const showAccountNFTsList = useMemo(() => {
    return viewAll ? myCreateNftList : myCreateNftList.slice(0, 4)
  }, [myCreateNftList, viewAll])
  ...
  return (
    <>
      ...
      >
        {showAccountNFTsList.map((item, index) => (
          <NFTItem6551 key={index} nft={item} idx={index}></NFTItem6551>
        ))}
        {!isSmDown && !viewAll && myCreateNftList.length > 4 && (
          <StyledItems
            sx={{
              display: 'flex',
              flexDirection: 'column',
...
function NFTItem6551({ nft, idx }: { nft: MyCreateNftListProp; idx: number }) {
  const isSmDown = useBreakpoint('sm')
  const [hoverIndex, setHoverIndex] = useState<any>(null)
  const refreshCb = useRefreshNft()
  const { result: nftInfo } = useNftAccountInfo(nft.tokenContract, nft.chainId)

  const refresh = useCallback(() => {
    refreshCb(nft.tokenContract, nft.chainId.toString()).then((res: any) => {
      if (res.data.code !== 200) {
        toast.error(res.data.msg || 'Refresh error')
        return
      }
  ...
  }, [nft.chainId, nft.tokenContract, refreshCb])
  ...
  return (
    ...
    >
      ...
      <Image
        altSrc={nftInfo?.logo_url || placeholderImage}
        style={{
          width: '100%',
          height: isSmDown ? 124 : 248,
          borderRadius: '10px 10px 0 0',
        ...
        <Link
          ...
          href={
            nft.chainId === ChainId.MAINNET
              ...
              : getEtherscanLink(nft.chainId, nft.transactionHash, 'transaction')
          }
        >
          <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
